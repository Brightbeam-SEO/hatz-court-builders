...
import {
  getBoisePowerServicePageConfig,
  type BoisePowerServiceSlug,
  isBoisePowerServiceSlug,
} from "@/lib/boise-power-service-page-config";
...
export async function getBoisePowerServicePageMetadata(slug: BoisePowerServiceSlug): Promise<Metadata> {
  const cfg = getBoisePowerServicePageConfig(slug);
  if (!cfg) return {};
...
export async function renderBoisePowerServicePage(slug: string) {
  if (!isBoisePowerServiceSlug(slug)) return null;
  const cfg = getBoisePowerServicePageConfig(slug);
  if (!cfg) return null;
