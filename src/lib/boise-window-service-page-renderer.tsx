...
import {
  getBoiseWindowServicePageConfig,
  type BoiseWindowServiceSlug,
  isBoiseWindowServiceSlug,
} from "@/lib/boise-window-service-page-config";
...
export async function getBoiseWindowServicePageMetadata(slug: BoiseWindowServiceSlug): Promise<Metadata> {
  const cfg = getBoiseWindowServicePageConfig(slug);
  if (!cfg) return {};
...
export async function renderBoiseWindowServicePage(slug: string) {
  if (!isBoiseWindowServiceSlug(slug)) return null;
  const cfg = getBoiseWindowServicePageConfig(slug);
  if (!cfg) return null;
